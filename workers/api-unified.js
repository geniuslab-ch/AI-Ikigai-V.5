...
async function handleSendInvitation(request, env, corsHeaders) {
    try {
        ...
        // ✨ NOUVEAU: Créer une relation coach-client en attente dans Supabase
        if (coachId && invitationToken) {
            try {
                ...
                if (existingClient) {
                    // Client existe déjà → créer relation directe
                    ...
                    if (relationError) {
                        ...
                        console.log('✅ Relation coach-client créée (client existant):', invitationId);
                    }
                } else {
                ...
                console.log(`📧 Invitation créée: coach_id=${coachId}, invitation_id=${invitationId}, email=${to}`);
            } catch (supabaseError) {
                console.error('⚠️ Erreur Supabase (non bloquant):', supabaseError);
                // Ne pas bloquer l'envoi de l'email si la création de relation échoue
            }
        }
